...
process_project_list() {
    ...
    if [ -z "$list_file_path" ]; then
        log_message ERROR "No input file path provided. Please specify the path to the list of projects. Example: ./project_list_processor.sh /path/to/your/project_list.txt <frequency>"
        return 1
    fi

    # 2. Validate target frequency argument
    if [ -z "$target_frequency" ]; then
        log_message ERROR "No backup frequency provided. Please specify one (e.g., 'hourly'). Allowed frequencies: ${allowed_frequencies[*]}."
        return 1
    elif [ "$is_valid_frequency" -eq 0 ]; then
        log_message ERROR "Invalid backup frequency provided: '$target_frequency'. Allowed frequencies: ${allowed_frequencies[*]}."
        return 1
    fi

    # 3. Validate input file existence
    if [ ! -f "$list_file_path" ]; then
        log_message ERROR "Input project list file not found: '$list_file_path'. Please ensure the file exists and the path is correct."
        return 1
    fi
    ...
    echo "----------------------------------------"
    echo "Projects to be Processed for frequency: '$target_frequency'"
    echo "----------------------------------------"
    printf "%-50s | %s\n" "Project Name" "Status"
    echo "----------------------------------------"
    ...
    log_message INFO "Loading project audit protocol from:"
    echo "'$PREFLIGHT_SCRIPT_PATH'..."
    if [ ! -f "$PREFLIGHT_SCRIPT_PATH" ]; then
        log_message ERROR "Script not found at '$PREFLIGHT_SCRIPT_PATH'. Please verify the script exists at the specified path."
        return 1
    fi
    # shellcheck source=/dev/null
    source "$PREFLIGHT_SCRIPT_PATH" || {
        log_message ERROR "Failed to source script file from '$PREFLIGHT_SCRIPT_PATH'. Please check the script for syntax errors or permission issues."
        return 1
    }
    echo ""
    log_message INFO "Project audit logic script loaded successfully..."
    echo "" # Add a blank line for readability

    log_message INFO "Loading backup mechanic script from:"
    echo "'$ACTUAL_BACKUP_PATH'..."
    if [ ! -f "$ACTUAL_BACKUP_PATH" ]; then
        log_message ERROR "Script not found at '$ACTUAL_BACKUP_PATH'. Please verify the script exists at the specified path."
        return 1
    fi
    source "$ACTUAL_BACKUP_PATH" || {
        log_message ERROR "Failed to source script file from '$ACTUAL_BACKUP_PATH'. Please check the script for syntax errors or permission issues."
        return 1
    }
    log_message INFO "Backup mechanic script loaded successfully..."
    echo "" # Add a blank line for readability

    log_message INFO "Loading filename generator script from:"
    echo "'$FILENAME_GENERATOR_PATH'..."
    if [ ! -f "$FILENAME_GENERATOR_PATH" ]; then
        log_message ERROR "Script not found at '$FILENAME_GENERATOR_PATH'. Please verify the script exists at the specified path."
        return 1
    fi
    source "$FILENAME_GENERATOR_PATH" || {
        log_message ERROR "Failed to source script file from '$FILENAME_GENERATOR_PATH'. Please check the script for syntax errors or permission issues."
        return 1
    }
    log_message INFO "Filename generator script loaded successfully..."
    echo "" # Add a blank line for readability

    log_message INFO "Loading cloud storage operations script from:"
    echo "'$STORAGE_CLOUD_PATH'..."
    if [ ! -f "$STORAGE_CLOUD_PATH" ]; then
        log_message ERROR "Script not found at '$STORAGE_CLOUD_PATH'. Please verify the script exists at the specified path."
        return 1
    fi
    source "$STORAGE_CLOUD_PATH" || {
        log_message ERROR "Failed to source script file from '$STORAGE_CLOUD_PATH'. Please check the script for syntax errors or permission issues."
        return 1
    }
    log_message INFO "Cloud storage operations script loaded successfully..."
    echo "" # Add a blank line for readability

    log_message INFO "Loading file operations script from:"
    echo "'$FILE_OPERATIONS_PATH'..."
    if [ ! -f "$FILE_OPERATIONS_PATH" ]; then
        log_message ERROR "Script not found at '$FILE_OPERATIONS_PATH'. Please verify the script exists at the specified path."
        return 1
    fi
    source "$FILE_OPERATIONS_PATH" || {
        log_message ERROR "Failed to source script file from '$FILE_OPERATIONS_PATH'. Please check the script for syntax errors or permission issues."
        return 1
    }
    log_message INFO "File operations script loaded successfully..."
    echo "" # Add a blank line for readability

    # --- End Sourcing ---


    log_message INFO "Starting processing each project for frequency: '$target_frequency'"
    echo "Project List File: '$(basename "$list_file_path")'"
    echo ""
    echo "----------------------------------------------------------------"
    # 4. Loop through the array for main processing
    if [ "$report_total_listed_files" -eq 0 ]; then
        log_message INFO "No projects to process from the list."
    else
        for current_project_path in "${project_paths_array[@]}"; do
            ...
            if [ "$project_preflight_status" -eq 0 ]; then
                log_message INFO "Project preflight for '$project_name' PASSED."
                ...
                case "$target_frequency" in
                    ...
                    "yearly")   current_run_timestamp_timestamp_string=$(date -d "1 year ago" +"%Y") ;;
                ...
                log_message INFO "Generated TIMESTAMP_STRING for '$project_name', frequency '$target_frequency': '$TIMESTAMP_STRING' (reflects previous period)."
                ...
                if [ -z "${DBS_TO_BACKUP[*]}" ]; then
                    log_message WARN "No databases specified in DBS_TO_BACKUP for project '$project_name'. Skipping backup for this project and frequency."
                ...
                for db_name_to_backup in "${DBS_TO_BACKUP[@]}"; do
                    echo ""
                    log_message INFO "Attempting backup for project '$project_name', database: '$db_name_to_backup', frequency: '$target_frequency'"
                    ...
                    if [ "$actual_backup_exit_status" -eq 0 ] && [ -n "$new_local_backup_file_path" ]; then
                        log_message INFO "Backup for project '$project_name', database '$db_name_to_backup' (frequency: '$target_frequency') completed successfully. File: '$new_local_backup_file_path'"
                        DATABASE_BACKUP_SUMMARY+=("$project_name|$db_name_to_backup|SUCCESS|$new_local_backup_file_path")
                    else
                        log_message ERROR "Backup for project '$project_name', database '$db_name_to_backup' (frequency: '$target_frequency') FAILED (Actual Backup Exit Status: $actual_backup_exit_status)."
                ...
                if [[ "$BACKUP_TYPE" == "cloud" || "$BACKUP_TYPE" == "both" ]]; then
                    echo ""
                    log_message INFO "Performing centralized cloud operations for project '$project_name', frequency '$target_frequency'..."
                    ...
                    # Check if there were any successful local backups to sync
                    local successful_local_backups_for_project=0
                    for entry in "${DATABASE_BACKUP_SUMMARY[@]}"; do
                        if [[ "$entry" == "$project_name|$db_name_to_backup|SUCCESS|"* ]]; then
                            successful_local_backups_for_project=1
                            break
                        fi
                    done

                    if [ "$successful_local_backups_for_project" -eq 0 ]; then
                        log_message WARN "No successful local database backups for project '$project_name' to synchronize to cloud. Skipping cloud operations."
                    else
                        ...
                        if [[ "$BACKUP_TYPE" == "both" ]]; then
                            log_message INFO "Cloud provider is '$CLOUD_STORAGE_PROVIDER' and BACKUP_TYPE is 'both'. Synchronizing local directory '$local_source_dir_for_sync' to cloud path '$s3_target_path_for_sync'."
                            if sync_to_cloud_s3 "$local_source_dir_for_sync" "$s3_target_path_for_sync"; then # Function name remains for compatibility
                                log_message INFO "Centralized cloud synchronization completed successfully for project '$project_name', frequency '$target_frequency'."
                                cloud_ops_successful=1
                            else
                                log_message ERROR "Centralized cloud synchronization failed for project '$project_name', frequency '$target_frequency'."
                                project_has_db_backup_failures=1 # Mark project as failed due to cloud sync failure
                            fi
                        elif [[ "$BACKUP_TYPE" == "cloud" ]]; then
                            log_message INFO "Cloud provider is '$CLOUD_STORAGE_PROVIDER' and BACKUP_TYPE is 'cloud'. Uploading new files only from '$local_source_dir_for_sync' to '$s3_target_path_for_sync'."
                            ...
                            if s3_upload_only_sync "$local_source_dir_for_sync" "$s3_target_path_for_sync"; then
                                log_message INFO "All new files successfully uploaded to cloud."
                                cloud_ops_successful=1
                            else
                                log_message ERROR "Some new files failed to upload to cloud. Cloud backup for this run may be incomplete."
                        ...
                        if [ "$cloud_ops_successful" -eq 1 ]; then
                            log_message INFO "Performing cloud cleanup with retention "$retention_minutes" minutes in '$s3_target_path_for_sync'."
                            if ! cleanup_cloud_s3 "$s3_target_path_for_sync" "$retention_minutes" "*.zip"; then # Function name remains for compatibility
                                log_message WARN "Cloud cleanup failed for '$s3_target_path_for_sync'."
                        ...
                        if [[ "$BACKUP_TYPE" == "cloud" ]]; then
                            log_message INFO "BACKUP_TYPE is 'cloud'. Removing all local backups from '$local_source_dir_for_sync' after successful cloud operations."
                            ...
                            if ! sudo find "$local_source_dir_for_sync" -maxdepth 1 -name "*.zip" -type f -print0 | xargs -0 sudo rm -f; then
                                log_message WARN "Failed to remove local files from '$local_source_dir_for_sync' after cloud operations."
                            else
                                log_message INFO "All local backups removed from '$local_source_dir_for_sync'."
                            fi
                        fi
                    fi # End of if successful_local_backups_for_project
                fi
                # --- End Centralized Cloud Sync and Final Local Cleanup ---

                if [ "$project_has_db_backup_failures" -eq 0 ]; then
                    log_message INFO "All database backups and cloud operations for project '$project_name' PASSED."
                    successful_projects=$((successful_projects + 1))
                    processed_project_statuses+=("PASSED")
                else
                    log_message ERROR "One or more database backups or cloud operations FAILED for project '$project_name'."
                    failed_projects=$((failed_projects + 1))
                    processed_project_statuses+=("FAILED")
                fi

            else # Project preflight failed
                log_message ERROR "Project preflight FAILED for '$project_name' (Exit Status: $project_preflight_status). Skipping database backups for this project."
                failed_projects=$((failed_projects + 1))
    ...
    echo "----------------------------------------------------------------"
    log_message INFO "Finished processing all project entries."
    log_message INFO "Summary: Total Projects Processed: $total_processed_projects, Successful: $successful_projects, Failed: $failed_projects"
    ...
    if [ "$failed_projects" -gt 0 ] || [ "$successful_projects" -eq 0 -a "$total_processed_projects" -gt 0 ]; then
        log_message ERROR "One or more projects or database backups failed. Please review the log messages and reports above for details."
        return 1
    else
        log_message INFO "All enabled projects and their database backups completed successfully."
        return 0
    fi
}
